...
psql -h localhost -p 5432 -U postgres ipdata -c "CREATE TABLE geoips(ipstart INET NOT NULL,ipend INET NOT NULL, country TEXT NOT NULL);"

psql -h localhost -p 5432 -U postgres ipdata -c "COPY geoips FROM '/tmp/ip2country-v4.tsv' DELIMITER E'\t';"
psql -h localhost -p 5432 -U postgres ipdata -c "COPY geoips FROM '/tmp/ip2country-v6.tsv' DELIMITER E'\t';"
#special case for when country is empty
psql -h localhost -p 5432 -U postgres ipdata -c "UPDATE geoips SET country='Unknown' WHERE country = '';"
...
filename text;
BEGIN
FOR rec IN SELECT DISTINCT country FROM geoips LOOP
filename := '/tmp/pgexport/' || rec.country;
EXECUTE format('COPY (SELECT inet_merge(ipstart,ipend) AS cidr FROM geoips WHERE country = %L ORDER BY cidr) TO %L', rec.country, filename);
END LOOP;
END $$;
EOF
